...
## 第 3 轮·P1 重跑量化对比（2026-07-12，带 stack-probe 的代码 vs 原 R3）
> 配置：代码=带 P1 主动栈探测(commit c559883+)；bank=bank-crack3.json(13题)；后端=lemonade Qwen3.6-35B；无引导基线；results-crack3-p1。判分：机械锚点判分器(离线)+ 强模型 rubric(云)双轨。

| 指标 | 原 R3(无栈探测) | R3-P1(带栈探测) | 读法 |
|---|---|---|---|
| rubric 均分 | 31.2 | **23.4** | 聚合反而降——**方差,非退步**(见下) |
| 机械锚点召回 | 0.32 | 0.258 | 同上,聚合被噪声淹 |
| 介入次数/题 | 2.3 | 2.2 | 持平 |
| 收尾率 | 10/13 | 10/13 | 持平 |

**关键结论(诚实):P1 在它针对的栈/边界题上大幅提升,但单跑聚合被 temp>0 双峰方差淹没,结论=单跑 A/B 无效。**
- **P1 针对题(同题对比,受控)全涨**:duolingo-1 栈边界 **10→46**、duolingo-4 il2cpp边界 **38→56**、kuwo-native-boundary **2→55**(原空壳→真分析+native边界诚实)。这些是 P1 该改善的题,确实改善。
- **崩题(非 P1 针对,纯方差)**:duolingo-2 **57→6**(这轮幻觉"JSON Patch 注入")、clone-02 **56→3**(截断)、clone-03 **88→0**(超时无输出)——这三题 P1 根本不碰,是同题不同跑的路径分叉(找到↔迷路双峰)。
- **两判分器(机械+rubric)一致给出"聚合下降",但拆开看栈题上升**——互相印证：**聚合指标在 temp>0 下是噪声**。

### ⚠️ 方法论铁律(直接约束记忆系统消融怎么跑)
**单跑聚合 A/B 无效。** 有效消融必须：(1) **同题多跑(≥3-5 seed)取中位数**;(2) **按题型分指标**(栈题看语义正确性/rubric,定位题看机械锚点召回,别混);(3) **对比受控到单变量**(同题、只改被测机制开/关)。这条已并入记忆系统 benchmark 设计,避免自欺。
