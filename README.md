...
**2. 模型能力边界（本地 35B）**：
  ...
  - 边缘（部分完成，靠安全网兜底或需人/强模型介入）：**多候选精确排除定位**、**重混淆下读不够时易幻觉**（已由"未读方法体不下结论 + 反幻觉铁律"显著缓解——找不到会说"未能证实"而非编造）、**组装完整多部分审计报告**（在强混淆样本上仍吃力）。
  - 单次运行**非确定**（temp>0）：同题不同跑结果有波动；安全网保证"稳定下限"（不超时/不跑飞/追对必收尾），非"每次满分"。

**3. 静态 + 只读 + 合规**：不做动态分析（frida/运行时 hook）；对反调试/root 检测/SSL pinning/完整性校验等只能**识别枚举**（防御审计视角），**不做绕过**；**不产出任何破解/重打包**。

### 🆘 何时需要人 / 更强模型介入
遇到上限（多候选难定位、组装不出完整报告、原地打转）时，用 `--ask-when-stuck`：agent 输出**详细困境报告**，你交给更强的模型取思路，再经 TUI 粘贴 / `--strategy` 反哺续跑。实测该闭环把独立做不出的硬题从 0 分补到满分（详见 `docs-resources`）。**这就是"本地小模型 + 人/强模型在环"做真实逆向的定位——不是全自动神器，而是可控、诚实、够得着题高效、够不着题会明说并求助的审计助手。**
