...
export function buildCorpusProtocol(m: CorpusManifest): string {
  ...
  return [
    ...
    '   · **锚点自检（防错锚点传染，最重要）**：案卷/前人给的每个 `file:line` 锚点，你 read 后要**逐字核对该处是否真如案卷所述**。不符就明确标『案卷此处有误：前人称 X，实读为 Y』，并自己重新定位真的证据点——**绝不照抄未经你亲自核对的锚点**（前人也可能弄错；照抄一个错锚点会让它传染成你的错结论）。案卷是"待核对的线索清单"，不是"免检的事实"。',
